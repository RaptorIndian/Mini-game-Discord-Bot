...
class Game:
    ...
    def __init__(self, names, controller = (1,0), difficulty = 1, debug = False):
        ...
        hands = [[],[]]
        for i in range(2):
            # Set first 7 cards of the deck to the hand
            hands[i] += self.deck[0:7]
            # Re-assign the rest of the deck
            self.deck = self.deck[7:]

        # Spawn Player instances
        for i in range(2):
            # Assign Player controllers to player members based on startup parameters
    ...
    def start(self):
        ...
        # Player object performing this turn's actions - Reassigned every time the active player switches
        target = None # Currently playing their turn
        other = None # The other player
        ...
        while self.active:
            ...
            # Check for end-state (all cards being matched)
            if target.empty_hand() and other.empty_hand() and len(self.deck) == 0:
                # Set the winner to whoever has more total matches
            ...
            if target.empty_hand() == False:
                # Have the player execute a turn and determine if a draw is needed
                turn = target.play_turn(other, self.turns, self.counter)
                draw = len(turn.cards) == 0

                # Draw if needed and possible
                if draw and self.deck:
                    print(target.id + " draws a " + self.deck[0]["text"])
                    target.draw(self.deck.pop(0))

                # Examine hand and parse through any matched cards
                target.process_matches()
            else:
                # No valid moves to be made due to an empty hand, so generate a null Turn
